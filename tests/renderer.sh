...
mkdir -p "$temp/state/sketchybar-agent-status" "$temp/config" "$temp/bin" "$temp/items"
...
XDG_STATE_HOME="$temp/state" XDG_CONFIG_HOME="$temp/config" SKETCHYBAR_BIN="$temp/bin/sketchybar" SKETCHYBAR_LOG="$temp/commands" FAKE_ITEMS="$temp/items" AGENT_STATUS_HOME="$root" AGENT_STATUS_POSITION=left "$root/scripts/agent_status.sh"
...
grep -F 'icon.font=SF Pro:Regular:10.0' "$temp/commands" >/dev/null
...
grep -F 'background.drawing=on background.color=0xff242426 background.border_width=0 background.height=20' "$temp/commands" >/dev/null
